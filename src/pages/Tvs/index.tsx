...
const TvsPage = () =>  {
  ...
  const { data, error, size, setSize } = useSWRInfinite(index => 
    `${BASE_URL}${endpoint}?page=${index+1}&${API_KEY}`, fetcher);
  ...
  return (
    <LayoutMain>
      ...
      <div className={cn("row no-gutters justify-content-between align-items-center mb-4")}>
      ...
      { data?.map(arr => <List list={arr} gridView={gridView} type='tv' />) }

      { size <= totalPages ? <button className="btn btn-primary" onClick={nextPage}>Load More</button> : null }
